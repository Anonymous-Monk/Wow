...
//静态广告
#define ImgUrlString1 @"http://liveimg.9158.com/pic/ad/20161118/201611181721292976.jpg"
//动态广告
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    _loginVC = [[UIStoryboard storyboardWithName:@"LoginSB" bundle:[NSBundle mainBundle]] instantiateInitialViewController];
    [AVOSCloud setApplicationId:@"mjKH3v4o3e3cRiL3ElHw6j1r-gzGzoHsz" clientKey:@"hA54HaimOKPxszCf9k1aH1hh"];
...
//                [weakLaunchAd presentViewController:VC animated:YES completion:nil];
        ...
        if (![[NSUserDefaults standardUserDefaults]boolForKey:@"isFirst"]) {
            self.window.rootViewController = [[RHGuideViewController alloc]init];
        }else{
...
-(void)requestImageData:(void(^)(NSString *imgUrl,NSInteger duration,NSString *openUrl))imageData{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if(imageData)
        {
            imageData(ImgUrlString1,5,@"http://www.returnoc.com");
        }
    });
}
