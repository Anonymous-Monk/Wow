...
static RHResponseType sg_responseType = kRHResponseTypeJSON;
static RHRequestType  sg_requestType  = kRHRequestTypeJSON;
...
@implementation RHNetworkingTools
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(RHURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[RHURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (RHURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(RHDownloadProgress)progress
                               success:(RHResponseSuccess)success
                                  fail:(RHResponseFail)fail {
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            RHLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
    ...
    RHURLSessionTask *session = nil;
    
    if (httpMethod == 1) {
        if (sg_cacheGet && !refreshCache) {// 获取缓存
            id response = [RHNetworkingTools cahceResponseWithURL:absolute
                                                   parameters:params];
            if (response) {
                if (success) {
...
+ (RHURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        RHLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        RHLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (RHURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    AFHTTPSessionManager *manager = [self manager];
    RHURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (RHURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                RHLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                RHLog(@"Download fail for url %@, reason : %@",
                          [self absoluteUrlWithPath:url],
                          [error description]);
            }
        }
...
+ (AFHTTPSessionManager *)manager {
    ...
    switch (sg_requestType) {
        case kRHRequestTypeJSON: {
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            break;
        }
        case kRHRequestTypePlainText: {
    ...
    switch (sg_responseType) {
        case kRHResponseTypeJSON: {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];
            break;
        }
        case kRHResponseTypeXML: {
            manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
            break;
        }
        case kRHResponseTypeData: {
...
+ (void)successResponse:(id)responseData callback:(RHResponseSuccess)success {
    if (success) {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        if (data) {
            cacheData = data;
            RHLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                RHLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                RHLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
...
+ (void)handleCallbackWithError:(NSError *)error fail:(RHResponseFail)fail {
    if ([error code] == NSURLErrorCancelled) {
