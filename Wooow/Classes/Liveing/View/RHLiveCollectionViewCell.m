...
@interface RHLiveCollectionViewCell ()
{
...
@property(nonatomic,strong) RHCarEarView *carEarView; //<<#description#>
@property(nonatomic,strong) RHLiveAuthorView *authorView; //<<#description#>
...
@implementation RHLiveCollectionViewCell
...
- (RHLiveAuthorView *)authorView{
    if (!_authorView) {
        RHLiveAuthorView *anchorView = [[RHLiveAuthorView alloc]initWithFrame:CGRectMake(0, 0, RHScreenWidth, 50)];

        [anchorView setClickFavAction:^(bool type) {
...
- (CAEmitterLayer *)emitterLayer
{
    if (!_emitterLayer) {
        ...
        [self.moviePlayer.view.layer insertSublayer:emitterLayer above:self.bottomToolView.layer];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _renderer.canvasMargin = UIEdgeInsetsMake(RHScreenHeight * 0.4, 10, 10, 10);
...
- (void)setLiveModel:(RHHotLiveModel *)liveModel{
    ...
    self.authorView.liveModel = liveModel;
...
- (void)didFinish
{
    ...
    [RHNetworkingTools getWithUrl:self.liveModel.flv refreshCache:YES success:^(id response) {
        NSLog(@"请求成功%@, 等待继续播放", response);
